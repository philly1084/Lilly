...
function normalizePiiFormulaPlanColumnId(value = '') {
    return String(value || '')
        .trim()
        .replace(/^['"`]+|['"`]+$/g, '')
        .replace(/[^\w.-]/g, '_')
...
function extractPiiFormulaPlanRowsSection(text = '') {
    const match = String(text || '').match(/\bRows\s*:\s*([\s\S]*?)(?:\n\s*Formula intent\s*:|\n\s*Target result cell\b|\n\s*Helper slots begin\b|$)/i);
    return match ? String(match[1] || '').trim() : '';
}

function buildPiiRelationshipFormulaPlanRequest(text = '') {
    const source = String(text || '').trim();
    if (!/\bxlsx_formula_plan\b/i.test(source)
        || !/\bTable id\s*:/i.test(source)
        || !/\bColumns\s*:/i.test(source)
        || !/\bRows\s*:/i.test(source)) {
        return null;
    }

    const tableId = normalizePiiFormulaPlanColumnId(source.match(/\bTable id\s*:\s*([^\r\n]+)/i)?.[1] || 'table');
    const columns = String(source.match(/\bColumns\s*:\s*([^\r\n]+)/i)?.[1] || '')
        .split(',')
        .map((entry) => normalizePiiFormulaPlanColumnId(entry))
        .filter(Boolean);
    const rows = extractPiiFormulaPlanRowsSection(source)
        .split(/\r?\n/)
        .map((line) => line.trim())
        .filter((line) => line && line.includes('|'))
        .map((line) => {
            const parts = line.split('|').map((part) => part.trim());
            const id = normalizePiiFormulaPlanColumnId(parts.shift() || '');
            const cells = {};
        ...
        .filter(Boolean);
    if (!tableId || columns.length === 0 || rows.length === 0) {
        return null;
    }
    ...
    const signedMeasures = Array.from(formulaIntent.matchAll(/([+-]?)\s*([A-Za-z_][\w.-]*)/g))
        .map((match) => ({
            sign: match[1] === '-' ? '-' : '+',
            column: normalizePiiFormulaPlanColumnId(match[2]),
        }))
        .filter((entry) => columns.includes(entry.column) && entry.column !== groupBy);
    const measures = Array.from(new Set(signedMeasures.filter((entry) => entry.sign !== '-').map((entry) => entry.column)));
    const parsedSubtractMeasures = Array.from(new Set(signedMeasures.filter((entry) => entry.sign === '-').map((entry) => entry.column)));
    const subtractMeasures = parsedSubtractMeasures.length > 0
        ? parsedSubtractMeasures
        : columns.filter((columnId) => /^credits?$/i.test(columnId));
    ...
    const sheetName = resultCell.includes('!') ? resultCell.split('!')[0].replace(/^'+|'+$/g, '') : undefined;

    return {
        operationId: 'orchestrator-xlsx-formula-plan',
...
function hasPiiRelationshipFormulaPlanIntentText(text = '') {
...
function shouldAllowManagedAppPlan({ objective = '', executionProfile = DEFAULT_EXECUTION_PROFILE, toolPolicy = {} } = {}) {
    ...
    return executionProfile === REMOTE_BUILD_EXECUTION_PROFILE
        || /\b(managed app|managed-app|build events webhook)\b/.test(normalized);
}
...
function inferResearchTimeRangeFromText(text = '') {
    return inferDefaultResearchTimeRange(text);
}
...
function isRemoteCliAgentHandoffOnlyTurn(text = '') {
    const normalized = String(text || '').trim().toLowerCase();
    if (!normalized) {
        return false;
    }

    return /^(?:please\s+)?(?:(?:can|could|would)\s+you\s+)?(?:use|try|call)\s+(?:the\s+)?remote[-_\s]+cli(?:[-_\s]+agent)?(?:\s+(?:please|instead|now))?[.!?]*$/.test(normalized);
}

function hasProjectAuthoringRequest(text = '') {
    const normalized = String(text || '').trim().toLowerCase();
    if (!normalized) {
        return false;
    }

    return /\b(?:build|create|develop|generate|implement|launch|make|publish|put|ship)\b/.test(normalized)
        && /\b(?:app|application|dashboard|demo|frontend|page|service|site|website)\b/.test(normalized);
}

function resolveRemoteObjectiveFromSession(rawObjective = '', session = null, recentMessages = []) {
    const agentHandoffOnly = isRemoteCliAgentHandoffOnlyTurn(rawObjective);
    if (!isRemoteApprovalOnlyTurn(rawObjective) && !agentHandoffOnly) {
        return rawObjective;
    }

    const controlState = getSessionControlState(session);
    const storedObjective = String(controlState.lastRemoteObjective || '').trim();
    if (storedObjective && !agentHandoffOnly) {
        return storedObjective;
    }
    if (storedObjective && hasProjectAuthoringRequest(storedObjective)) {
        const checkpointSummary = String(controlState?.userCheckpoint?.lastResponse?.summary || '').trim();
        return [
            `Original unfinished request: ${storedObjective}`,
            ...(checkpointSummary ? [`Confirmed user checkpoint: ${checkpointSummary}`] : []),
            `Current instruction: ${rawObjective}`,
        ].join('\n');
    }
    ...
    if (agentHandoffOnly) {
        for (let index = transcript.length - 1; index >= 0; index -= 1) {
            ...
            if (!candidate
                || candidate === rawObjective
                || isRemoteCliAgentHandoffOnlyTurn(candidate)
                || !hasProjectAuthoringRequest(candidate)) {
                continue;
            }

            const checkpointSummary = String(controlState?.userCheckpoint?.lastResponse?.summary || '').trim();
            return [
                `Original unfinished request: ${candidate}`,
                ...(checkpointSummary ? [`Confirmed user checkpoint: ${checkpointSummary}`] : []),
                `Current instruction: ${rawObjective}`,
            ].join('\n');
        }
...
function hasRemoteCliAgentAuthoringIntent(text = '', { executionProfile = '' } = {}) {
    ...
    const deploymentIntent = remoteSandboxTarget
        || /\b(deploy|deployment|redeploy|publish|launch|ship|go live|get (?:it|the app|the site|the website) (?:live|online|deployed)|bring (?:it|the app|the site|the website) (?:live|online)|route|ingress|tls|dns|domain|rollout)\b/.test(normalized);
...
function hasExplicitDirectRemoteCliIntent(text = '') {
    ...
    return [
        ...
        /\b(?:without|bypass|skip|do not use|don't use|no|never)\s+(?:the\s+)?(?:gitlab|managed[- ]app|managed app)\b/,
...
function shouldAttachActiveProjectArtifact(objective = '') {
    const normalized = String(objective || '').trim().toLowerCase();
    if (!normalized) {
        return false;
    }
    if (/\b(?:new|another|different|fresh)\b[\s\S]{0,40}\b(?:app|artifact|game|page|project|site|website)\b/.test(normalized)
        || /\b(?:from scratch|start over)\b/.test(normalized)) {
        return false;
    }
    return /\b(?:continue|deploy|finish|fix|improve|iterate|polish|publish|redeploy|refine|resume|retry|ship|update)\b/.test(normalized)
        || /\b(?:bring|go|make|put|take)\b[\s\S]{0,30}\b(?:live|online|server)\b/.test(normalized)
        || /\bbuild\b[\s\S]{0,40}\b(?:artifact|current|existing|selected|that|this)\b/.test(normalized);
}

function collectRemoteCliAgentArtifactIds({
    objective = '',
    session = null,
    toolContext = {},
    params = {},
} = {}) {
    const explicitArtifactIds = [
    ...
    if (explicitArtifactIds.length > 0) {
        return Array.from(new Set(explicitArtifactIds));
    }

    const activeProject = session?.metadata?.activeProject;
    const projectType = String(activeProject?.type || '').trim().toLowerCase();
    if (!['managed-app', 'sandbox'].includes(projectType)
        || !shouldAttachActiveProjectArtifact(objective)) {
        return [];
    }
    ...
    return sourceArtifactId ? [sourceArtifactId] : [];
}
...
function hasManagedAppAuthoringIntent(text = '', options = {}) {
    ...
    return appContext
        && changeIntent
        && remoteContext
        && (explicitManagedAppContext || managedAppArtifactPublish);
}
    ...
    return {
        ...
        remote: remoteRelevant
            ? [
                ...
                'The default remote-cli-agent transport uses the authenticated `/admin/remote-agent-tasks` provider lane for Codex and Kimi; direct `/api/codex-agent` and `remote_code_run`/`remote_code_status` remain explicit compatibility lanes.',
...
class ConversationOrchestrator extends EventEmitter {
    ...
    buildToolPolicy({
        ...
        const hasRemoteCliAgentMetadataPreference = preferredMetadataToolId === 'remote-cli-agent'
            || String(metadata?.directToolId || toolMetadata.directToolId || '').trim() === 'remote-cli-agent'
        ...
        if (executionProfile === REMOTE_BUILD_EXECUTION_PROFILE) {
            ...
            if ((hasRemoteCliAgentAuthoringRequest || hasExplicitRemoteCliAgentRequest || hasRemoteCliAgentContinuationPreference) && allowedToolIds.includes('remote-cli-agent')) {
        ...
        if ((hasPiiFormulaPlanIntent || hasPiiWorkbookRelationshipRequest)
            ...
            candidateToolIds = [
                ...
                ...candidateToolIds.filter((toolId) => (
                    ...
                    && toolId !== DEEP_RESEARCH_PRESENTATION_TOOL_ID
                )),
            ];
        }
    ...
    buildDirectAction({ objective = '', session = null, recentMessages = [], toolPolicy = {}, toolContext = {}, toolEvents = [] }) {
        ...
        if (piiFormulaPlanRequest
            ...
            return finalizeAction({
                tool: RELATIONSHIP_CALCULATION_TOOL_ID,
                reason: 'PII spreadsheet formula-plan requests must use the relationship calculator instead of document or artifact generation.',
                params: piiFormulaPlanRequest,
            });
        }
        ...
        const prompt = [
            ...
            ...(toolPolicy?.rolePipeline
                ? [
                    ...
                    'Do not force a fixed set of controls or effects. Ask the builder to create metadata.handoff.buildWorkbench when the task is really an agent build flow: phases, commands, hookPoints, callableHooks for scripts/functions, objectFactories, qaGates, and fallback decisions.',
    ...
    buildRuntimeInstructions({
        ...
        if (toolPolicy?.rolePipeline) {
            ...
            if (toolPolicy.rolePipeline.requiresSandbox && allowedToolIds.includes('code-sandbox')) {
                ...
                parts.push('Let the builder create a request-specific build workbench rather than forcing stock UI examples. Preserve phases, commands, script/function hooks, object factories, QA gates, and repair/redesign decisions in metadata.handoff.buildWorkbench so later agents can continue the same build.');
        ...
        if (toolPolicy?.remoteCliInventorySummary) {
            ...
            if (allowedToolIds.includes('remote-cli-agent')) {
                parts.push('Remote tool boundary: call the outer `remote-cli-agent` tool with a prose `task`; do not pass `command`, `args`, `shell`, `executable`, or a direct transport wrapper such as `remote_code_run` as its params.');
