  ...
  registerSystemTools() {
    ...
    const mediaTools = [
      ...
      },
      {
        ...
        name: 'Podcast',
        category: 'system',
        description: 'Research a topic or use supplied files/source documents, script a user-aligned podcast episode, synthesize the host voices with local TTS, stitch the final podcast audio into a saved artifact, and optionally render an MP4 podcast video. Podcast audio is speaker-only by default; when the user asks to use admin, uploaded, saved, or configured podcast audio sources, set voiceOnlyAudio:false and includeIntro/includeOutro/includeMusicBed as requested so those admin assets are mixed under the finished recording before video rendering. Preserve the full creative brief separately from the topic, including solo-vs-two-host format, required angle, facts, title, exclusions, and any selected scriptDesign/scriptDesignExample. For two-host podcasts, make the hosts feed off each other: answer the previous point, add a sharper angle, challenge a lazy assumption, ask a concrete next question, and use subject-appropriate jokes only when they naturally clarify or release tension. For a training podcast, set scriptDesign:"training-podcast", prefer one calm instructor speaker unless the user asks for multiple speakers, use source documents/files as the curriculum, and set useOnlineResearch:false when those files are sufficient. Prefer proper full scripts over short generic exchanges. Avoid over-reflection and repeated self-referential process language about dissecting, unpacking, cadence, human rhythm, or why the hosts sound human. Visual podcast requests should use storyboard mode with content-matched infographic scenes; waveform-card is the simple audio visualizer fallback.',
        backend: {
          handler: async (params = {}, context = {}) => {
    ...
    const skillTools = [
      {
        ...
        category: 'system',
        description: 'Apply a bounded self-reflection update to Hermes-style soul/user cards/files, durable carryover notes, registered skills, and model-card audit notes.',
        backend: {
            ...
            actions: {
              ...
              items: {
                ...
                properties: {
                  type: {
                    type: 'string',
                    description: 'One of soul_append, user_profile_append, agent_notes_append, soul_patch, user_profile_patch, agent_notes_patch, soul_replace, user_profile_replace, agent_notes_replace, carryover_notes_replace, skill_patch, skill_update, skill_create, or model_card_note.',
                  },
                  reason: { type: 'string' },
                  content: { type: 'string' },
                  notes: { type: 'string' },
                  ...
                  heading: { type: 'string' },
                  section: { type: 'string' },
                  compactedContent: { type: 'string' },
                  compacted_content: { type: 'string' },
        ...
        skill: {
          triggerPatterns: [
            ...
            'update the soul card',
            'grow with our interactions',
            'agents growing with interactions',
            'model card update',
            'update user files',
            'update the skill',
