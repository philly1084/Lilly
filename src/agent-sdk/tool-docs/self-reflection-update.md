...
- `oldText` and `newText`: exact text replacement for `skill_patch`.
  ...
  "actions": [
    {
      ...
      "reason": "Update compact carryover guidance.",
      "content": "# Carryover Notes\n\n## Workflow Preferences\n- Use bounded self-reflection updates only for stable lessons that improve future work.\n"
    },
    {
...
- Use `soul_replace` and `user_profile_replace` only with the full replacement file content, not a partial fragment.
- Use notes replacement only with the complete compact notes content, not a partial fragment.
