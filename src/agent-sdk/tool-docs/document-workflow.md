...
- For interactive HTML documents, dashboards, graph explorers, 3D explainers, code viewers, and document viewers, prefer local sandbox browser library routes from `/api/sandbox-libraries/` before external CDNs. Common choices include Three.js, Chart.js, D3, Mermaid, Cytoscape, Plotly, ECharts, vis-network, GSAP, Matter.js, p5.js, Rough.js, Force Graph, 3D Force Graph, CodeMirror, highlight.js, Marked, PDF.js, Mammoth, and docx.js.
- For code/document viewer suites, build an actual work surface rather than a static dump: tabs or file tree, line numbers, syntax highlighting, rendered Markdown, outline/search, PDF/DOCX import preview where relevant, copy/download controls, and loading/empty/error states.
