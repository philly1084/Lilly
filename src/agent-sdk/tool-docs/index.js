...
const TOOL_SUPPORT = {
  ...
  'remote-tools': { status: 'stable', notes: ['Compact lane picker for the unified remote operations system: managed-app, remote-cli-agent, remote-command, remote-workbench, k3s-deploy, the shared provider-task contract, and explicit direct Codex/MCP compatibility contracts.'] },
  ...
  'game-studio': { status: 'stable', notes: ['Durable Lilly-owned multi-genre browser game projects with blank, third-person explorer, top-down action, and procedural expedition templates; component controllers; agent-authored module manifests; typed systems; mechanics; prefab variants; materials; uploaded asset metadata; animation controllers; terrain heightfields; deterministic specifications; scenes/components; Blueprints; transactional commands; immutable builds; managed-app publishing; and rollback.', 'Use list-templates before create-project when the requested genre is not explicit. The same workflow is available to remote coding agents through the lilly-game CLI.', 'Requires GAME_STUDIO_ENABLED=true; disposable game prototypes remain on code-sandbox.'] },
  'file-search': { status: 'stable', notes: ['Finds local runtime files by glob pattern so agents can inspect existing source before editing.', 'Use this as the first step in IDE-style frontend follow-ups instead of regenerating whole files.'] },
  'file-read': { status: 'stable', notes: ['Reads selected local runtime files after discovery.', 'Use targeted reads to preserve reusable frontend source and avoid broad context dumps.'] },
  'file-write': { status: 'stable', notes: ['Writes a full local runtime file body and mirrors session artifacts when possible.', 'Requires path and content in the same call; for reusable frontend work, search/read first and prefer targeted edits over unnecessary full rewrites.'] },
