...
const TOOL_SUPPORT = {
  ...
  'remote-tools': { status: 'stable', notes: ['Compact lane picker for remote-cli-agent, remote-command, remote-workbench, k3s-deploy, the preferred /api/codex-agent run/events contract, and legacy remote_code_run/status boundaries.'] },
  ...
  'remote-cli-agent': {
    ...
    notes: [
      'Server-side Codex agent integration for the gateway /api/codex-agent/run plus /events SSE contract, with legacy remote_code_run/status MCP compatibility.',
      'Preferred setup uses REMOTE_CLI_AGENT_TRANSPORT=codex-agent plus REMOTE_CLI_CODEX_AGENT_BASE_URL or GATEWAY_URL and a trusted bearer key; legacy MCP still uses REMOTE_CLI_MCP_URL plus REMOTE_CLI_MCP_BEARER_TOKEN or N8N_API_KEY.',
