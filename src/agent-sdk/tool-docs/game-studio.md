...
Use `save-game-design` with `productionId`, current `revision`, complete `plan`, and optional team settings to persist review edits without starting model workers. Use the returned revision for subsequent saves or starts. This action is available only before building, while the production is awaiting review.
