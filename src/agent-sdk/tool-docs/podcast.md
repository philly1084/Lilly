...
- Podcast renders use the curated high-quality female voice pool by default: Kokoro `af_heart`, `af_bella`, `af_nicole`, and `bf_emma`, with female Piper fallback aliases such as `lessac-high`, `ljspeech-high`, and `cori-high` mapped during rollout.
...
- Example: `hostAVoiceIds: ["af_heart", "af_bella"]` and `hostBVoiceIds: ["af_bella", "af_nicole", "bf_emma"]` lets the same host cycle through the highest-quality bundled female Kokoro voices per turn.
