...
- Remote CLI iteration final output must include `WHAT_CHANGED`, `VERIFY_COMMANDS`, `VERIFY_RESULTS`, `PUBLIC_URL`, and `BLOCKER` markers. `BLOCKER=none` means the run reached its intended stopping point; any other blocker should be treated as incomplete until the user or platform resolves it.
