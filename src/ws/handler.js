...
function buildWsCanvasInstructions(canvasType = 'document', existingContent = '', requestPrompt = '') {
    ...
    const typeGuides = {
        ...
        diagram: 'Generate Mermaid syntax for ordinary diagram requests. If the request mentions Excalidraw, canvas-excalidraw, editable board objects, selected object ids, canvasContext, or an actionContract, return /api/canvas-compatible JSON where content is an inner board-edit JSON string using add, add_many, update, update_many, delete, and select actions. Use editable rectangle/diamond/ellipse/arrow/line/freedraw/text/sticky/frame/storyboardFrame/animationBeat/audioCue/mermaidDiagram objects. For storyboardFrame or animationBeat include title, text, startTime, and durationSeconds. For audioCue include title, text, audioName, startTime, and durationSeconds. For mermaidDiagram include title and mermaidSource. Do not create raster screenshots, image snapshots, or image elements unless the user explicitly asks for an image asset.',
