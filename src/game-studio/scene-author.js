...
function validatePlayableScene(candidate, plan) {
  ...
  for (const entity of scene.entities.filter(entity => component(entity, 'Camera'))) {
    const rotation = component(entity, 'Transform')?.data.rotation || {};
    if (Object.values(rotation).some(value => Math.abs(Number(value)) > Math.PI * 2)) throw invalid(`Camera ${entity.id} rotation must use normalized radians (-2*pi to 2*pi), not degrees. For a 48.8 degree downward tilt use rotation.x=-0.851720675. Fixed cameras retain their authored transform.`);
  }
