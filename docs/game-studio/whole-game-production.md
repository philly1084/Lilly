...
REST mirrors the tools at `/api/game-studio/production-capabilities`, `/productions`, `/productions/:id`, and `/productions/:id/save|start|resume|stop`. The capability manifest is versioned separately from plans and outputs. Future workers should add validated output adapters and advertise them there; do not infer support from a model's name, bypass revision checks, or substitute prose for an asset or passing test.

Before building, use **Save design** or `save-game-design` with `productionId`, the current `revision`, the complete `plan`, and optional `models`, `taskModels`, and `concurrency`. This persists the reviewed design and team without calling models or creating a project. It returns a new revision. Saving and starting share worker ownership; stale revisions are rejected rather than overwriting another session. Designs cannot be saved after building begins.
