...
- Default transport: KimiBuilt calls authenticated `POST /admin/remote-agent-tasks`, streams the returned same-origin task URL, and collects gateway-verified result files when `REMOTE_CLI_AGENT_TRANSPORT=provider-agent`. The router maps OpenAI/Codex, Kimi, and Grok model families to their bounded CLI providers, stages operation-scoped handoff files, preserves supported provider `sessionId` values, and keeps the privileged task route cluster-internal.
