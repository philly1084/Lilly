...
The remaining release proofs are served-surface interaction through Canvas, Notes, and Web CLI, a successful run against an authorized disposable host, and the actual lockstep router/KimiBuilt rollout. No production live canary or rollout has been performed. A green PR, dry run, mocked canary test, queued deployment, or passing preflight must not be described as a live deployment.
...
8. Run `npm run canary:sandbox-agent-attach -- --run --mode all`. Require the real sandbox ZIP, all nine non-admin runs, both typed attachment failures, exact destination checksums, and terminal-gated cleanup.
