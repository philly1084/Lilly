...
`POST /api/artifacts/:id/managed-app/preflight` runs the same ownership, archive, path, member, binary-asset, privacy-restoration, and final-byte preparation used by Push to Web without calling `createApp` or mutating a repository. It reports source eligibility separately from repository readiness: valid final bytes can pass `managedAppSourcePreflight` while `pushToWebEligibility` remains blocked by typed repository-control-plane facts. Ordinary transfer and authoring validation requires the source gate; only an explicit Push-to-Web request requires repository authentication and managed-app organization access. The response contains only these safe readiness facts, typed blockers, target paths, byte counts, per-file SHA-256 values, and one aggregate source SHA-256; it never returns source content or credentials. `GET /api/managed-apps/readiness` exposes the non-mutating repository gate before an artifact exists.
...
The router and KimiBuilt lockstep rollout, exact Codex/Kimi transfer proof, both CLI authoring/browser proofs, and the Codex sandbox-origin/Canvas/Notes API continuity run were completed in production on 2026-07-18/19 UTC. The deployed `sha-db040d5` sandbox canary streamed `SandboxAgentAttachProgress/v1` heartbeats through all three Codex runs and passed with one exact 2,961-byte ZIP (`c59ee783de8d172147142a9e02547b7c5a793e0ebc8233823b44f9de6317da9e`) across sandbox origin, Canvas, and Notes. Its final cleanup proof reported three deleted ephemeral sessions, one deleted sandbox workspace, and no retained session or workspace IDs; the former artifact, preview route, and workspace path all returned absent after cleanup.

The deployed `sha-b09af05` Kimi authoring canary ran in a disposable, label-scoped Kubernetes Job with 30-second `KimiAuthoringCanaryJob/v1` heartbeats and completed in 1,096 seconds with exit code zero. Both exact transfer hops and original authoring passed; each stage verified four component descriptors, four downloaded files, a four-file site ZIP, preview equality, live remote execution, and the `kimi-code-cli`/`k3` identity. Authoring additionally passed the local structural gate, semantic brief, and two clean browser viewports. The source preflight passed while Push-to-Web eligibility remained separately blocked by `MANAGED_APP_REPOSITORY_AUTH_REJECTED`. Cleanup then proved the ephemeral session and authored bundle returned HTTP 404, the async run was terminal `completed`, and the exact Job and pod were absent.

The two-lane canary candidate at `2b89bdeb` completed the missing Kimi sandbox-origin/Canvas/Notes continuity proof against the production `sha-42be992` runtime on 2026-07-19 UTC. A read-only copy of the candidate script ran in the label-scoped Job `kimi-sandbox-attach-proof-20260719-0225`; the shared backend and router stayed on their deployed images. All three non-admin Kimi runs returned `kimi-code-cli`/`k3` and preserved one exact 2,948-byte ZIP (`38f10082ad9e57e0f295da53681973455f48619505b47905e0aab22b329deba6`) across sandbox origin, destination-owned Canvas attachment/return, and destination-owned Notes attachment/return. Both wrong-session/wrong-surface gates returned `ARTIFACT_ATTACH_TARGET_SESSION_MISMATCH`, all async runs reached `completed`, and cleanup deleted three sessions plus the exact sandbox workspace with no retained IDs. The Job, pod, ConfigMap, and remote staging files were then removed.

Served-surface proof completed on 2026-07-19 UTC. One authenticated browser run moved the same 2,365-byte ZIP (`dee3ff274b39776b2ee0aedb84a0b00d15c62f7a80c72bcbd1dd9b4dbb0dacf8`) through the deployed Canvas, Notes, and Web CLI surfaces, proved each surface used its destination-owned artifact ID, captured all three screenshots, and deleted all four ephemeral sessions with the source artifact and workspace absent afterward. After the query-driven Web CLI mirror shipped in `sha-61c7c58`, a second authenticated served-route canary opened Web CLI with only source lineage parameters. The page itself called the shared attach contract, rewrote the URL to destination artifact `e3a35c9e-d546-4959-91c3-821fd742f76b`, selected that destination for Codex/Kimi, enabled `Build Selected (1)`, preserved the exact 2,948-byte source checksum (`38f10082ad9e57e0f295da53681973455f48619505b47905e0aab22b329deba6`), and reported zero console or page errors. Cleanup returned HTTP 404 for the source artifact, destination artifact, and sandbox workspace.

Grok is explicitly excluded from the delivery scope and acceptance criteria. The only remaining release proof is one successful Push-to-Web run against an authorized disposable host after GitLab authentication is repaired. The latest readiness attempt still fails closed before agent execution or repository mutation with `MANAGED_APP_REPOSITORY_AUTH_REJECTED` because the configured GitLab credential returns HTTP 401. A green PR, dry run, mocked canary test, queued deployment, source preflight, or repository-readiness check must not be described as a live deployment.
