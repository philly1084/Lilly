...
- For print/PDF output, include print-safe styles with dark text on light backgrounds unless a dark printed panel is explicitly defined. Default PDF pages should be portrait `11.33in x 14.67in` with comfortable border room, such as `@page { size: 11.33in 14.67in portrait; margin: 0.72in 0.65in 0.68in; }`, unless the user asks for another size or the artifact is explicitly a landscape slide/deck. Avoid viewport-only sizing, prevent orphan headings where practical, and ensure table rows, figures, and callouts do not split awkwardly.
