...
- For routine public web research, start with Perplexity-backed `web-search`; use raw search for candidate URL discovery and `pro-search` for researched synthesis when snippets/headlines would be too thin
...
When an agent needs a small workflow doc instead of the full project instructions, start with `docs/agent-grep/`. Treat this file as the top-level map; use compact docs such as `docs/agent-grep/tool-use-rhythm.md`, `docs/agent-grep/remote-tools.md`, `docs/agent-grep/build-and-verify.md`, and `docs/agent-grep/notes-databases.md` for the active workflow before loading longer runbooks.
...
- Before creating or provisioning a new website, app, dashboard, service, GitLab project, namespace, or public host, run an inventory check and prove it does not already exist. Check managed-app list/inspect results, configured GitLab projects, cluster continuity or project registry facts, live k3s namespaces/deployments/services/ingresses, and the requested hostname or slug. If there is a reasonable match by name, slug, domain, repo, namespace, or purpose, reuse or iterate the existing project, or ask before creating a duplicate.
