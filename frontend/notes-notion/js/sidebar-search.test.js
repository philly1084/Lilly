...
describe('Notes settings dialog accessibility', () => {
    ...
    test('exposes each visible settings group as a dialog heading', () => {
        const source = readSidebarSource();
        ...
        [
            ...
            expect(source).toContain(`<div class="settings-section-title" role="heading" aria-level="2">${label}</div>`);
        });
    });
});
...
describe('Notes sidebar outline heading clarity', () => {
    test('keeps long heading labels readable within two compact lines', () => {
        const html = fs.readFileSync(path.join(__dirname, '..', 'index.html'), 'utf8');
        const styles = fs.readFileSync(path.join(__dirname, '..', 'css', 'styles.css'), 'utf8');
...
describe('Notes sidebar action hierarchy', () => {
    test('keeps Import visually neutral until hover instead of resembling the selected page', () => {
        const html = fs.readFileSync(path.join(__dirname, '..', 'index.html'), 'utf8');
        const styles = fs.readFileSync(path.join(__dirname, '..', 'css', 'styles.css'), 'utf8');
