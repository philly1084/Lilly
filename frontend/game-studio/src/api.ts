...
export const studioApi = {
  ...
  createProject: (input: { name: string; slug?: string; project?: unknown; importBundle?: unknown }) => request<StudioProjectResponse>('/api/game-studio/projects', { method: 'POST', body: JSON.stringify(input) }),
  ...
  proposeAi: (projectId: string, baseRevision: number, prompt: string) => request<AiRun>(`/api/game-studio/projects/${encodeURIComponent(projectId)}/ai-runs`, { method: 'POST', body: JSON.stringify({ baseRevision, prompt }) }),
