...
export type GamePlan = { schema: 'LillyGamePlan/v1'; name: string; fantasy: string; artDirection: string; coreLoop: string[]; winCondition: string; loseCondition: string; controls: string[]; acceptance: string[]; deferred: string[]; levelPrompt: string; environmentPrompt: string; gameplayPrompt: string; assets: Array<{ id: string; name: string; prompt: string; placement: string }> };
export type GameProduction = {
  id: string; brief: string; revision: number; status: string; createdAt: string; projectId: string | null;
  plan: GamePlan | null; models: Record<string, string>; concurrency: number; build?: StudioBuild;
...
export function GameProductionCreator({ model, models }: { model: string; models: Array<{ id: string; name?: string }> }) {
  ...
  const busy = pending || running(production);
  ...
  useEffect(() => {
    ...
    productionApi.list().then(result => {
      ...
      if (latest) { setProduction(latest); setAssignments(latest.models); setConcurrency(latest.concurrency); }
  ...
  return <div className="level-creator game-production">
    ...
    {productions.length > 0 && <label className="creator-model">Saved game builds<select value={production?.id || ''} disabled={pending} onChange={event => {
      if (!event.target.value) { setProduction(null); setAssignments({}); return; }
      perform(async () => { const value = await productionApi.get(event.target.value); setAssignments(value.models); setConcurrency(value.concurrency); return value; });
    ...
    {production && <>
      ...
      {plan && <section className="production-plan" aria-label="Game design">
        <h3>{plan.name}</h3><p>{plan.fantasy}</p>
      ...
      {production.status === 'review' && <button type="button" className="creator-generate" disabled={pending} onClick={() => perform(() => productionApi.control(production.id, 'start', { revision: production.revision, plan: JSON.parse(planText), models: selectedModels, concurrency }))}>Build this game</button>}
      {['failed', 'stopped', 'interrupted'].includes(production.status) && production.error?.code !== 'REVISION_CONFLICT' && <button type="button" className="creator-generate" disabled={pending} onClick={() => perform(() => productionApi.control(production.id, 'resume', { revision: production.revision, models: selectedModels, concurrency }))}>Retry unfinished work</button>}
      ...
      {!running(production) && <button type="button" disabled={pending} onClick={() => { setProduction(null); setAssignments({}); setError(''); }}>New game idea</button>}
    </>}
